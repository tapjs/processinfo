...
t.cleanSnapshot = s => s.split(esmURL).join('{ESMURL}')
  .split(esmRel.replace(/"/g, '\\"')).join('{ESMREL}')
  .split(esmAbs.replace(/"/g, '\\"')).join('{ESMABS}')
  .split(cjsAbs.replace(/"/g, '\\"')).join('{CJSABS}')
  .split(cjsRel.replace(/"/g, '\\"')).join('{CJSREL}')
  .split(cwdEnc.replace(/"/g, '\\"')).join('{CWD}')
...
const cases = {
  ...
  'exp opts has esm url': [{NODE_OPTIONS: ['--experimental-loader', esmURL]}, []],
  'exp opts has esm url =': [{NODE_OPTIONS: `--experimental-loader=${esmURL}`}, []],
  'exp opts has esm relative': [{NODE_OPTIONS: [`--experimental-loader=${esmRel}`]}, []],
  'exp opts has esm absolute': [{NODE_OPTIONS: ['--experimental-loader', esmAbs]}, []],
  ...
  'opts esm, args cjs': [{NODE_OPTIONS: ['-r', cjsRel]}, ['--loader=' + esmURL]],
  'opts cjs, args cjs': [{NODE_OPTIONS: ['--loader=' + esmURL]}, ['-r', cjsRel]],

  'other loader in opts': [{NODE_OPTIONS:['--loader',__filename]}, []],
  'other loader in args': [{}, ['--loader',__filename]],
  'other loader in both': [{NODE_OPTIONS:['--loader',__filename]}, ['--loader',__dirname + '/index.cjs']],
  'multiple loaders': [{NODE_OPTIONS:['--loader', __filename, '--loader',__dirname + '/index.cjs']},[]],
