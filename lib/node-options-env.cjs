...
const addCJS = args => !hasCJSLoader(args)
  ...
  : args
const addESM = args => squashLoaders(
  !hasESMLoader(args)
...
const resolveLoader = path => {
  try {
    return require.resolve(
      ...
      : /^file:/.test(path) ? fileURLToPath(path) // unpossible?
      : path
    )
  } catch (e) {
    return path
...
const squashLoaders = args => {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    const resolved = resolveLoader(decode(val, true))
    if (resolved === esmLoader) {
      continue
    }
    loaders.push(encodeURIComponent(val))
  }
  squashed.push(`${esmLoader}?${loaders.join('&')}`)
  return squashed
}
